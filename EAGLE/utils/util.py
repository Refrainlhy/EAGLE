...
def compute_metrics(
    ...
    with torch.no_grad():
        ...
        # true_binary_label= torch.zeros(len(pred_pos))
        # pred_binary_label = torch.argmax(torch.cat([pred_pos,pred_neg],dim=1),dim=1).cpu().detach()
        ...
        for i in range(num_edge):
            # Extract relevant pred_neg values for each positive prediction
            neg_samples = [pred_neg[j * num_edge + i] for j in range(num_neg)]
            combined_scores = torch.tensor(
                [pred_pos[i].item()] + [s.item() for s in neg_samples]
            ).to(device)

            # Sort scores in descending order and find the rank of the positive sample
            ...
            # Update HR@k for each k in k_list
            for idx, k in enumerate(k_list):
                if rank <= k:
                    hr_list[idx] += 1

        # Normalize MRR and HR@k by the number of edges
        mrr /= num_edge
...
class tppr_node_finder:
    ...
    # input: nodes = [source, target, fake, fake2, fake3, fake4, fake5]
    # input: timestamps = []
    # input: scores = [dst_score, neg_score, neg_score]
    # input: dict_list = []
    def precompute_link_prediction(self, source_nodes, num_neg):
        ...
        """
    dict_list = typed.List()
    for _ in range(len(source_nodes)):
      tppr_dict = nb.typed.Dict.empty(
        key_type=types.int64,
        value_type=types.float64,
      )
      dict_list.append(tppr_dict)
    """
        ...
        for index0, alpha in enumerate(self.alpha_list):
            ...
            for i in range(n_edges):
                ...
                # dict_list[i]=PPR_list[source].copy()
                # dict_list[i+n_edges]=PPR_list[target].copy()
                ...
                for neg_index in range(num_neg):
                    ...
                    # dict_list[i+(neg_index+2)*n_edges]=PPR_list[fake]
                ...
                for index, pair in enumerate(pairs):
                    ...
                    ################# s2 side #################
                    if norm_list[s2] == 0:  # new node never appear before
