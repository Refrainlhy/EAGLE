...
from utils.util import set_random_seed, compute_metrics, print_model_info, EarlyStopMonitor, MYNegEdgeSampler
...
parser = argparse.ArgumentParser("Training time model")
parser.add_argument(
    "--dataset_name",
    type=str,
    default="wikipedia",
...
filename = (
    DATA + "_topk_" + str(args.topk) + "_flag_" + args.topk_sample_flag
    + "_lr_"
    + str(args.lr)
    + "_wd_"
...
def get_neighbor_finder(data):
    ...
    for neighbors in adj_list:
        ...
        node_to_edge_timestamps.append(
            np.array([x[2] for x in sorted_neighhbors], dtype=np.float64)
        )
...
class NeighborFinder:
    ...
    # return delta time in increasing order
...
print(
    f"\nFinal result will be written in: log_learn_time/{DATA}/{filename}\n"
)
...
train_neg_edge_sampler = MYNegEdgeSampler(destinations=train_data.destinations, full_destinations=train_data.destinations, num_neg=1, device=device, seed=2024)
val_neg_edge_sampler = MYNegEdgeSampler(destinations=val_data.destinations, full_destinations=full_data.destinations, num_neg=1, device=device, seed=2025)
test_neg_edge_sampler = MYNegEdgeSampler(destinations=test_data.destinations, full_destinations=full_data.destinations, num_neg=99, device=device, seed=2026)
...
def run(
    ...
    k_list = [1, 2, 3, 4, 5, 10, 20, 30, 40, 50, 60, 70, 80],
):
    if mode == "Train":
        model = model.train()
    else:
    ...
    for batch_idx in tqdm(range(num_batch)):
        ...
        if mode == "Train" and no_neighbor_flag == False:
            ...
            loss = creterion(input=predicts, target=labels)
            loss.backward()
            optimizer.step()
            t_epo += time.time() - t2
        elif mode == "Val":
...
with torch.no_grad():
    _, t_test4, test_all_pos_score, test_all_neg_score, mem_test = run(
        model,
        "Test",
    ...
    print(
        f"\nNum_epochs: {num_epo}, total_train_time: {t_train:4f}, total_val_time: {t_val:4f}, total_test_time_neg99: {t_test4:4f}, memory_train: {mem_train}, memory_val: {mem_val}, memory_test: {mem_test}."
    )

with open(f, "a") as log_file:
    log_file.write(
        f"Num_epochs: {num_epo}, total_train_time: {t_train:4f}, total_val_time: {t_val:4f}, total_test_time_neg99: {t_test4:4f}, memory_train: {mem_train}, memory_val: {mem_val}, memory_test: {mem_test}."
    )
